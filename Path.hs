...
aStarCtrl st@(PS 0   info PProcessNeighbor u v upC) cin@((Q.Out (Right Q.Pushing) _),_,_)            = PS 0    info (PError Unknown) u v upC-- finished reading memory, check whether poping is finished
...
aStarCtrl st@(PS mdc info PPushNeighbor u v upC) cin@_
    ...
    | otherwise = PS 1 info PPushNeighbor u v (upC + 1)
      where waiting = let qStatus ((Q.Out s _),_,_) = s
                       in (qStatus cin) /= Right Q.Ready || mdc > 0
aStarCtrl st@(PS mdc info PFinished u v upC) _ = PS 0 info PWaiting u v 0
